...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!empty($id_admin) && !empty($id_direksaun) && !empty($data_registu) && !empty($no_ref) && !empty($asuntu) && !empty($hato) && !empty($kategoria)) {
        ...
        // Insert data into tb_karta_tama
        $stmt = $conn->prepare("INSERT INTO tb_karta_tama (id_admin, id_direksaun, data_karta_tama, no_ref, asuntu, hato, kategoria) VALUES (?, ?, ?, ?, ?, ?, ?)");
        if ($stmt === false) {
        ...
        if ($stmt->execute()) {
            echo "<script>alert('Dadus aumenta ona!'); document.location.href='?pagekarta=karta_tama';</script>";
        } else {
